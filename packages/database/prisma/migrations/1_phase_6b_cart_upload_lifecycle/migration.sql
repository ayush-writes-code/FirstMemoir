...
-- Safely migrate existing LOW_RESOLUTION records before altering the column type.
-- Since DPI validation is now handled at cart checkout rather than at upload time,
-- any previously processed image stuck in LOW_RESOLUTION is now considered READY.
UPDATE "user_uploads" SET "status" = 'READY'::"UploadStatus" WHERE "status" = 'LOW_RESOLUTION';
