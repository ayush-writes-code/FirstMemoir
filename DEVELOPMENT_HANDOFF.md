...
5. **Cross-Site Auth**: Vercel (`.vercel.app`) and Render (`.onrender.com`) constitute a cross-site architecture. The API must be deployed with `COOKIE_SAME_SITE=none` so that mobile browsers don't silently block `HttpOnly` credentials. Normal environments retain the safer default. The actual deployment has not yet occurred.
